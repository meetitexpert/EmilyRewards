...
export class ApisService {
  ...
  getApiVersion(apiName: string) {
    ...
    if(apiName == this.constants.getRecommentedPromotions){
      api_version = 2
    }

    if (apiName == this.constants.getCategories || apiName == this.constants.getProducstsList || apiName == this.constants.getProductDetail || apiName == this.constants.getJournalData ) {
      api_version = 3
    }
